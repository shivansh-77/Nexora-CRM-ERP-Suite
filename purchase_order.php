...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Retrieve form data for new columns (Vendor Details)
    ...
    // Fetch vendor_id based on vendor_name
    ...
    // Generate the new purchase order number before updating item_ledger_history
    $last_purchase_order_query = "
        SELECT COALESCE(MAX(CAST(SUBSTRING(purchase_order_no, 8) AS UNSIGNED)), 0) AS last_purchase_order_no
        FROM purchase_order
        WHERE purchase_order_no LIKE 'PO/$currentYear/%'
    ";
    $last_purchase_order_result = $connection->query($last_purchase_order_query);
    $last_purchase_order_no = $last_purchase_order_result->fetch_assoc();

    // Calculate the new sequential number
    $new_sequence_no = $last_purchase_order_no['last_purchase_order_no'] + 1;

    // Format the new purchase order number
    $purchase_order_no = 'PO/' . $currentYear . '/' . str_pad($new_sequence_no, 4, '0', STR_PAD_LEFT);

    // Insert data into the purchase_order table
    $insert_purchase_order = "INSERT INTO purchase_order (vendor_name, shipper_location_code,
        purchase_order_no, vendor_id, shipper_id, gross_amount, discount,
        net_amount, purchase_order_date, total_igst, total_cgst, total_sgst, base_amount,
        vendor_address, vendor_phone, vendor_city, vendor_state, vendor_country,
        vendor_pincode, vendor_gstno, shipper_company_name, shipper_address,
        shipper_city, shipper_state, shipper_country, shipper_pincode,
        shipper_phone, shipper_gstno, fy_code
    ) VALUES ('$vendor_name', '$shipper_location_code',
        '$purchase_order_no', '$vendor_id', '$shipper_id', '$gross_amount',
        '$discount', '$net_amount', '$purchase_order_date', '$total_igst', '$total_cgst',
        '$total_sgst', '$base_amount', '$vendor_address', '$vendor_phone',
        '$vendor_city', '$vendor_state', '$vendor_country', '$vendor_pincode',
        '$vendor_gstno', '$shipper_company_name', '$shipper_address', '$shipper_city',
        '$shipper_state', '$shipper_country', '$shipper_pincode', '$shipper_phone',
        '$shipper_gstno', '$fy_code'
    )";

    if ($connection->query($insert_purchase_order) === TRUE) {
        $purchase_order_id = $connection->insert_id;
        ...
        for ($i = 0; $i < count($products); $i++) {
            ...
            $cgst = $cgsts[$i];
            $sgst = $sgsts[$i];
            $receipt_date = $receipt_dates[$i]; // Get receipt date

            // Insert into purchase_order_items
            $insert_item = "INSERT INTO purchase_order_items (purchase_order_id, product_name, product_id, quantity, rate, gst, amount, unit, value, igst, cgst, sgst, receipt_date) VALUES
                ('$purchase_order_id', '$product_name', '$product_code', '$quantity', '$rate', '$gst', '$amount', '$unit', '$unit_value', '$igst', '$cgst', '$sgst', '$receipt_date')";
            $connection->query($insert_item);
        }

        echo "<script>alert('Purchase Order Saved!'); window.location.href='purchase_order_display.php';</script>";
    } else {
        echo "<p>Error saving purchase order: " . $connection->error . "</p>";
    }
}
?>
...
<body>
  ...
  <div class="container">
      <a style="text-decoration: none; margin-left: 1137px; padding: 0px; position: relative; top: -34px; transform: translateY(-50%);" href="purchase_order_display.php" class="close-btn">&times;</a>
      ...
      <form id="purchaseOrderForm" method="POST" action="">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-top: -25px; width: 100%;">
              <div style="flex-grow: 1; text-align: center;">
                  <h1 style="margin-left: 150px;">Purchase Order</h1>
              </div>
          ...
          <table id="purchaseOrderTable" border="1" style="width: 100%; border-collapse: collapse; margin-top: 20px;">
              <thead>
                  <tr>
                      <th>Product</th>
                      <th>Unit</th>
  ...
  <script>
      ...
      function addRow() {
          ...
          row.innerHTML = `
              ...
              <td><input type="date" name="receipt_date[]" onchange="calculateRow(this)" required></td> <!-- Input for receipt date -->
      ...
      function calculateTotal() {
          let rows = document.querySelectorAll("#purchaseOrderTable tbody tr");
          let gross = 0;
      ...
      function updateGSTTotals() {
          ...
          document.querySelectorAll("#purchaseOrderTable tbody tr").forEach(row => {
