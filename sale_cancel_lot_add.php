...
$invoice_item_id = isset($_GET['invoice_item_id']) ? intval($_GET['invoice_item_id']) : 0;
$product_id = isset($_GET['product_id']) ? $_GET['product_id'] : '';

// Handle lot deletion via AJAX
if(isset($_POST['delete_lot']) && isset($_POST['lot_id'])) {
    $lot_id = intval($_POST['lot_id']);
    $delete_query = "DELETE FROM cancelled_item_lot WHERE id = ?";
...
$item_query = "SELECT pi.*, p.invoice_no, p.shipper_location_code
               FROM invoice_items_cancel pi
               JOIN invoices_cancel p ON pi.invoice_id = p.id
               WHERE pi.id = ?";
...
$existing_lots = [];
$lots_query = "SELECT * FROM cancelled_item_lot
               WHERE invoice_itemid = ?
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['delete_lot'])) {
    ...
    if ($total_quantity != $item['stock']) {
        $error = "Error: Total quantity ($total_quantity) must match stock ({$item['stock']})";
    }
    // Check for duplicate lot numbers
        ...
        $delete_query = "DELETE FROM cancelled_item_lot WHERE invoice_itemid = ?";
        $stmt_delete = $connection->prepare($delete_query);
        ...
        for ($i = 0; $i < count($quantities); $i++) {
            ...
            $insert_query = "INSERT INTO cancelled_item_lot
    ...
    echo '<script type="text/javascript">
            ...
            window.location.href = "invoice_cancel_form_display.php?id=' . $invoice_id . '";
          </script>';
    exit();
        }
    }
...
<html>
<head>
    <title>Add Sales Invoice Cancel Lot Details</title>
    <style>
        body {
...
<script>
    document.getElementById('crossBtn').addEventListener('click', function() {
        ...
        window.location.href = 'invoice_cancel_form_display.php?id=' + encodeURIComponent(invoiceId);
    });
</script>
    <div class="info-container">
        ...
        <p><strong>Quantity to Assign:</strong> <?= htmlspecialchars($item['stock']) ?></p>
    ...
    <script>
        ...
        document.getElementById('lotForm').addEventListener('submit', function(e) {
            ...
            if (total !== stock) {
                e.preventDefault();
                alert(`Total quantity (${total}) must match stock (${stock})`);
                return false;
            }
