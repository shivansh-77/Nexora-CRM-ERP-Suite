...
<link rel="icon" type="image/png" href="favicon.png">
  ...
  <body>
    <div class="leadforhead">
      <h2 class="leadfor">Lead For</h2>
      <div class="lead-actions">
        <div class="search-bar">
          <input type="text" id="searchInput" class="search-input" placeholder="Search...">
          <button class="btn-search" id="searchButton">🔍</button>
        </div>
        <a href="lead_for_add.php">
    <button class="btn-primary" id="openModal" data-mode="add" title="Add New Lead">➕</button>
</a>
    ...
    <div class="user-table-wrapper">
      <table class="user-table">
          ...
          <tbody>
              <?php
              // Fetch data from the lead_for table
              $query = "SELECT * FROM lead_for";
              $result = mysqli_query($connection, $query);

              if (mysqli_num_rows($result) > 0) {
                  while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr ondblclick=\"window.location.href='lead_for_edit.php?id={$row['id']}'\" style='cursor: pointer;'>

                              <td>" . ($row['name'] ?? 'N/A') . "</td>
                              <td>" . ($row['status'] ?? 'N/A') . "</td>
                              <td>
      <button class='btn-warning edit-btn' title='Update this Lead'
            onclick=\"window.location.href='lead_for_edit.php?id={$row['id']}';\">✏️</button>
      <button class='btn-danger' title='Delete this Lead'
            onclick=\"if(confirm('Are you sure you want to delete this record?')) {
                window.location.href='lead_for_delete.php?id={$row['id']}';
            }\">🗑️</button>
  </td>
                            </tr>";
                  }
