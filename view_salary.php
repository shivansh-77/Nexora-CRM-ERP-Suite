...
if ($is_edit) {
    ...
    $edit_query = "SELECT * FROM salary WHERE id = ?";
    $stmt = mysqli_prepare($connection, $edit_query);
    mysqli_stmt_bind_param($stmt, "i", $_GET['id']);
    mysqli_stmt_execute($stmt);
    $edit_result = mysqli_stmt_get_result($stmt);

    if ($existing_data = mysqli_fetch_assoc($edit_result)) {
        // Data found, we'll use it to populate the form
        $employee_id = $existing_data['employee_id'];
        $month = $existing_data['month'];
        $fy_code = $existing_data['fy_code'];
        $month_id = $existing_data['month_id'];
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $month_id = $is_edit ? $existing_data['month_id'] : $_GET['month_id']; // From URL parameters or existing data

    // Get employee name and salary from login_db
    $employee_query = "SELECT name, salary FROM login_db WHERE id = ?";
    $stmt = mysqli_prepare($connection, $employee_query);
    ...
    $working_days = floatval($is_edit ? $existing_data['working_days'] : ($_POST['working_days'] ?? 0));
    $weekends = floatval($is_edit ? $existing_data['weekends'] : ($_POST['weekends'] ?? 0));
    $holidays = floatval($is_edit ? $existing_data['holidays'] : ($_POST['holidays'] ?? 0));
    $sick_leaves = floatval($is_edit ? $existing_data['sick_leaves'] : ($_POST['sick_leaves'] ?? 0));
    $earned_leaves = floatval($is_edit ? $existing_data['earned_leaves'] : ($_POST['earned_leaves'] ?? 0));
    $half_days = floatval($is_edit ? $existing_data['half_days'] : ($_POST['half_day'] ?? 0));
    $lwp = floatval($is_edit ? $existing_data['lwp'] : ($_POST['lwp'] ?? 0));
    $payable_days = floatval($is_edit ? $existing_data['payable_days'] : ($_POST['payable_days'] ?? 0));
    $total_days = floatval($is_edit ? $existing_data['total_days'] : ($_POST['total_days'] ?? 0));
    $salary_per_day = floatval($is_edit ? $existing_data['salary_per_day'] : ($_POST['salary_per_day'] ?? 0));
    $payable_salary = floatval($is_edit ? $existing_data['payable_salary'] : ($_POST['payable_salary'] ?? 0));
    ...
    if ($is_edit) {
        ...
        mysqli_stmt_bind_param($stmt, "ddi",
            $additional_fund, $total_amount, $_GET['id']);
    } else {
        // Record doesn't exist, insert new (your original code)
    ...
    if (mysqli_stmt_execute($stmt)) {
        $_SESSION['success'] = "Salary record " . ($is_edit ? "updated" : "saved") . " successfully";
...
// Check if the required parameters are set in the URL or we're editing
if ($is_edit || (isset($_GET['fy_code']) && isset($_GET['month_id']) && isset($_GET['month']) && isset($_GET['start_date']) && isset($_GET['end_date']))) {
    if (!$is_edit) {
...
<head>
    ...
    <title><?php echo $is_edit ? 'Edit' : 'Create'; ?> Salary Sheet</title>
    <style>
        body {
            background: #2c3e50;
...
<body>
  ...
  <div class="container">
    <a href="salary_sheets.php?fy_code=<?php echo urlencode($fy_code); ?>&month_id=<?php echo $month_id; ?>&month=<?php echo urlencode($month); ?>" class="cross-btn">✖</a>
      <div class="title">
          <span><?php echo $is_edit ? 'Edit' : 'Create'; ?> Salary Sheet</span>
      </div>
      <form action="" method="POST">
          <div class="form-grid">
              ...
              <div class="input_field">
                  ...
                  <select name="employee_name" id="employee_name" required <?php echo $is_edit ? 'disabled' : ''; ?>>
                      <option value="">Select Employee</option>
                      <?php foreach ($employees as $employee): ?>
                          <option value="<?php echo $employee['id']; ?>"
                                  data-salary="<?php echo $employee['salary']; ?>"
                                  <?php if ($is_edit && $employee['id'] == $existing_data['employee_id']) echo 'selected'; ?>>
                              <?php echo $employee['name']; ?>
                          </option>
                      <?php endforeach; ?>
                  </select>
                  <?php if ($is_edit): ?>
                      <input type="hidden" name="employee_name" value="<?php echo $existing_data['employee_id']; ?>">
                  <?php endif; ?>
              </div>
              ...
              <div class="input_field">
                  ...
                  <input type="text" name="month" id="month" value="<?php echo htmlspecialchars($is_edit ? $existing_data['month'] : $month); ?>" readonly>
                  <span id="month-warning" style="color: red; display: none;">Salary sheet already created for this month.</span>
              </div>

              <!-- Hidden FY Code -->
              <input type="hidden" name="fy_code" value="<?php echo htmlspecialchars($is_edit ? $existing_data['fy_code'] : $fy_code); ?>">

              <!-- Working Days -->
              <div class="input_field">
                  <label for="working_days">Working Days</label>
                  <input type="number" name="working_days" id="working_days" value="<?php echo $is_edit ? $existing_data['working_days'] : ''; ?>" readonly>
              </div>

              <!-- Weekends -->
              <div class="input_field">
                  <label for="weekends">Weekends</label>
                  <input type="number" name="weekends" id="weekends" value="<?php echo $is_edit ? $existing_data['weekends'] : ''; ?>" readonly>
              </div>

              <!-- Holidays -->
              <div class="input_field">
                  <label for="holidays">Holidays</label>
                  <input type="number" name="holidays" id="holidays" value="<?php echo $is_edit ? $existing_data['holidays'] : ''; ?>" readonly>
              </div>

              <!-- Sick Leaves -->
              <div class="input_field">
                  <label for="sick_leaves">Sick Leaves</label>
                  <input type="number" name="sick_leaves" id="sick_leaves" value="<?php echo $is_edit ? $existing_data['sick_leaves'] : ''; ?>" readonly>
              </div>

              <!-- Earned Leaves -->
              <div class="input_field">
                  <label for="earned_leaves">Earned Leaves</label>
                  <input type="number" name="earned_leaves" id="earned_leaves" value="<?php echo $is_edit ? $existing_data['earned_leaves'] : ''; ?>" readonly>
              </div>

              <!-- Half Day -->
              <div class="input_field">
                  <label for="half_day">Half Day</label>
                  <input type="number" name="half_day" id="half_day" value="<?php echo $is_edit ? $existing_data['half_days'] : ''; ?>" readonly>
              </div>

              <!-- Leave Without Pay -->
              <div class="input_field">
                  <label for="lwp">Leave Without Pay</label>
                  <input type="number" name="lwp" id="lwp" value="<?php echo $is_edit ? $existing_data['lwp'] : ''; ?>" readonly>
              </div>

              <!-- Total Days -->
              <div class="input_field">
                  <label for="total_days">Total Days</label>
                  <input type="number" name="total_days" id="total_days" value="<?php echo $is_edit ? $existing_data['total_days'] : ''; ?>" readonly>
              </div>

              <!-- Payable Days -->
              <div class="input_field">
                  <label for="payable_days">Payable Days</label>
                  <input type="number" name="payable_days" id="payable_days" value="<?php echo $is_edit ? $existing_data['payable_days'] : ''; ?>" readonly>
              </div>

              <!-- Salary Per Day -->
              <div class="input_field">
                  <label for="salary_per_day">Salary Per Day</label>
                  <input type="number" name="salary_per_day" id="salary_per_day" value="<?php echo $is_edit ? $existing_data['salary_per_day'] : ''; ?>" readonly>
              </div>

              <!-- Payable Salary -->
              <div class="input_field">
                  <label for="payable_salary">Payable Salary</label>
                  <input type="number" name="payable_salary" id="payable_salary" value="<?php echo $is_edit ? $existing_data['payable_salary'] : ''; ?>" readonly>
              </div>

              <!-- Additional Fund -->
              <div class="input_field">
                  <label for="additional_fund">Additional Fund</label>
                  <input type="number" name="additional_fund" id="additional_fund" value="<?php echo $is_edit ? $existing_data['additional_fund'] : '0'; ?>" min="0">
              </div>

              <!-- Total Amount -->
              <div class="input_field">
                  <label for="total_amount">Total Amount</label>
                  <input type="number" name="total_amount" id="total_amount" value="<?php echo $is_edit ? $existing_data['total_amount'] : ''; ?>" readonly>
              </div>
          </div>
          ...
          <div class="btn-container">
            ...
            <?php endif; ?>
              ...
              <button type="submit" class="btn-register"><?php echo $is_edit ? 'Update' : 'Register'; ?></button>
  ...
  document.addEventListener('DOMContentLoaded', function() {
      ...
      // If editing, calculate total amount immediately
      if (<?php echo $is_edit ? 'true' : 'false'; ?>) {
          calculateTotalAmount();
      } else {
          // For new records, use the original logic
  ...
  function resetAllFields() {
      const fieldsToReset = [
          ...
          'earned_leaves', 'half_day', 'lwp', 'total_days',
  ...
  function fetchWorkingDays(employeeId, startDate, endDate) {
      return fetch('fetch_working_days.php', {
          method: 'POST',
          headers: {
  ...
  function fetchLeaves(employeeId, startDate, endDate) {
      ...
      .then(data => {
          ...
          document.getElementById('half_day').value = data.halfDay || 0;
      })
      .catch(error => {
          console.error('Error:', error);
