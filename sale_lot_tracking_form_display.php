    ...
    if ($invoice) {
        ...
        $items_query = "
            ...
            FROM
                invoice_items ii
            LEFT JOIN
                item i ON ii.product_id = i.item_code
            WHERE
                ii.invoice_id = ?
        ";
...
<head>
    ...
    <title>Sales Lot Tracking Form</title>
    <style>
        body {
...
<body>
  <div class="invoice-container">
      ...
      <div class="header" style="position: relative;">
          ...
          <h1>Sales Lot Tracking Form</h1>
          <a href="invoice_add_lot_display.php" class="close-button" style="position: absolute; top: -21px; right: -17px; text-decoration: none; color: #333; font-size: 20px; font-weight: bold;">×</a>
      </div>

      <div class="invoice-info">
          <p><strong>Invoice Reference:</strong> <?php echo isset($invoice['id']) ? htmlspecialchars($invoice['id']) : ''; ?></p>
              ...
              <?php if (!empty($items)) { ?>
                  ...
                  ?>
                      <tr class="<?php echo $isEditable ? 'editable' : 'uneditable'; ?>">
                          ...
                          <td>
                              <?php if ($isEditable) { ?>
                                  <a href="sale_lot_add.php?invoice_id=<?php echo $invoice_id; ?>&item_id=<?php echo htmlspecialchars($item['id']); ?>&product_id=<?php echo htmlspecialchars($item['product_id']); ?>">
    ...
    function validateAndSave() {
        ...
        fetch('validate_sale_lot_details.php?invoice_id=<?php echo $invoice_id; ?>')
            .then(response => response.json())
            .then(data => {
                if (data.valid) {
                    if (confirm('Are you sure you want to finalize this invoice?')) {
                        // Make a fetch call to finalize_invoice.php
                        fetch('finalize_sale_invoice.php?invoice_id=<?php echo $invoice_id; ?>')
                            .then(response => response.json())
                            .then(result => {
