...
if (isset($_GET['pdf_mode'])) {
    // Force print styles when generating PDF
    $_GET['print'] = 'true';
...
if (isset($_GET['id'])) {
    ...
    if ($invoice) {
      ...
      $stmt_hsn->close();
        ...
        // First fetch invoice_no from invoices table
        $invoice_no = '';
        $invoice_query = "SELECT invoice_no FROM invoices WHERE id = ?";
        $stmt_invoice = $connection->prepare($invoice_query);
        $stmt_invoice->bind_param("i", $invoice_id);
        $stmt_invoice->execute();
        $invoice_result = $stmt_invoice->get_result();
        ...
        if ($invoice_row && !empty($invoice_row['invoice_no'])) {
            $invoice_no = $invoice_row['invoice_no'];
        }

        // Fetch terms and conditions with fallback logic
        $terms_content = '';
        // 1. First try to get invoice-specific terms using invoice_no as document_no
        $terms_query = "SELECT terms_and_conditions
                        FROM invoice_terms_conditions
                        WHERE document_no = ? AND type = 'Sales'";
        $stmt_terms = $connection->prepare($terms_query);
        $stmt_terms->bind_param("s", $invoice_no);
        $stmt_terms->execute();
        ...
        <div class="summary-row">
            <div class="terms-conditions">
                ...
                <form id="terms-form" method="POST" action="save_terms.php">
                    ...
                    <!-- Replace your current print button with this -->
  <button type="button" class="no-print" onclick="window.print()">
      <?php echo isset($_GET['pdf_mode']) ? 'Generating PDF...' : 'Print Invoice'; ?>
  </button>
                </form>
            </div>
            <div class="amount">
