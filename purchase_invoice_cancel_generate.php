...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$requires_lot_tracking) {
        $last_invoice_query = "SELECT MAX(CAST(SUBSTRING(invoice_no, 8) AS UNSIGNED)) AS last_invoice_no
                             FROM purchase_invoice WHERE invoice_no LIKE 'PSR/$currentYear/%'";
    ...
    $insert_invoice = "INSERT INTO purchase_invoice_cancel (
    ...
    if ($connection->query($insert_invoice) === TRUE) {
        ...
        for ($i = 0; $i < count($products); $i++) {
            ...
            $insert_item = "INSERT INTO purchase_invoice_cancel_items (
        ...
        if ($requires_lot_tracking) {
            header("Location: purchase_cancel_form_display.php?id=$purchase_invoice_id");
            exit();
        } else {
      ...
      <form id="invoiceForm" method="POST" action="">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-top: -25px; width: 100%;">
              <div style="flex-grow: 1; text-align: center;">
                  <h1 style="margin-left: 150px;">Purchase Invoice Cancel Generate</h1>
              </div>
