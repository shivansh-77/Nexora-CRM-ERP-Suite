...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $deleteQuery = "DELETE FROM gst WHERE id = $id";

    if (mysqli_query($connection, $deleteQuery)) {
        // Check if the deleted ID is the highest ID in the table
        $maxIdQuery = "SELECT MAX(id) AS max_id FROM gst";
        ...
        echo "<script>alert('Error deleting record: " . mysqli_error($connection) . "'); window.location.href='gst_display.php';</script>";
    }
} else {
