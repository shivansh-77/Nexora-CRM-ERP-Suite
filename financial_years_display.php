...
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>financial_year_display</title>
    <style>
    html, body {
        overflow: hidden;
        height: 100%;
  ...
  <body>
    <div class="leadforhead">
      <h2 class="leadfor">Financial Years Record</h2>
      <div class="lead-actions">
        <div class="search-bar">
          <input type="text" id="searchInput" class="search-input" placeholder="Search...">
          <button class="btn-search" id="searchButton">🔍</button>
        </div>
        <a href="add_financial_years.php">
      <button class="btn-primary" id="openModal" title="Add New Financial Year" data-mode="add">➕</button>
  </a>
      </div>
    </div>
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<tr>
                        ...
                        <td>
                            <input type='radio' name='current' class='current-checkbox'
                                data-id='{$row['id']}' $isChecked />
                        </td>
                        <td>
      <button class='btn-warning edit-btn' title='Manage FY Permissions'
          onclick=\"window.location.href='user_fy_permission.php?fy_code={$row['fy_code']}';\">🔒</button>
      <button class='btn-danger' title='Delete this Financial Year'
          onclick=\"if(confirm('Are you sure you want to delete this record?')) {
              window.location.href='delete_financial_years.php?id={$row['id']}';
          }\">🗑️</button>
  </td>
                      </tr>";
            }
        } else {
            echo "<tr><td colspan='5'>No financial Year records found</td></tr>";
        }
        ?>
    </tbody>
</table>
