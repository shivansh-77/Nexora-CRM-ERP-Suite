...
if (isset($_GET['invoice_id'])) {
    ...
    $check_status_query = "SELECT status, invoice_no FROM purchase_invoice WHERE id = ?";
    ...
    try {
        ...
        $last_invoice_query = "
            ...
            FROM purchase_invoice WHERE invoice_no LIKE ?
        ";
        $pattern = "PI/$currentYear/%";
      ...
      while ($item = $lot_items_result->fetch_assoc()) {
          $insert_ledger_stmt->bind_param(
              "sssssdssdissdi", // Added 'i' at the end for invoice_id_main (integer)
              $purchase_invoice_no,
              $doc_type,
              $entry_type,
              $item['product_id'],
              ...
              $item['rate'],
              $invoice_id // Added invoice_id_main value
          );
      ...
      $insert_ledger_stmt->close();
        ...
        $ledger_type = 'Vendor Ledger';
        $party_type = 'Vendor';
        $doc_type_party = 'Purchase Invoice';
