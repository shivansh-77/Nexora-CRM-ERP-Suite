...
<tbody>
    <?php
    $query = "SELECT * FROM user_leave WHERE user_id = ?";
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const userNameFilter = document.getElementById('userNameFilter').value.toLowerCase(); // New filter
        ...
        document.querySelectorAll('.user-table tbody tr').forEach(row => {
            ...
            const userNameText = row.children[1].textContent.trim().toLowerCase(); // New filter
            ...
            let showRow = (idFilter === '' || idText.includes(idFilter)) &&
                          (userNameFilter === '' || userNameText.includes(userNameFilter)) && // New filter
