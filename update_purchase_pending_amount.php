...
if ($updateSuccess) {
    ...
    $query_insert_ledger = "INSERT INTO party_ledger
                           ...
                           VALUES
                           ('Vendor Ledger', 'Vendor', 'Payment Paid', ?,
                            ?, ?, ?,
                            NOW(), ?, ?, ?)";
    ...
    // Insert the same record into advance_payments table immediately after party_ledger insert
    ...
    mysqli_stmt_bind_param($stmt_advance, 'sdissss',
        $invoice_no,
        $amountPaid,
        $vendor_id,
        $vendor_name,
    ...
    if (!$advance_result) {
        echo json_encode(['success' => false, 'error' => 'Advance payments insert failed: ' . mysqli_stmt_error($stmt_advance)]);
        exit;
    }
