...
<link rel="icon" type="image/png" href="favicon.png">
  ...
  <body>
    <div class="leadforhead">
      <h2 class="leadfor">HSN/SAC</h2>
      <div class="lead-actions">
        <div class="search-bar">
          <input type="text" id="searchInput" class="search-input" placeholder="Search...">
          <button class="btn-search" id="searchButton">🔍</button>
        </div>
        <a href="add_hsn_sac.php">
      <button class="btn-primary" id="openModal" title="Add New HSN/SAC" data-mode="add">➕</button>
  </a>
    ...
    <div class="user-table-wrapper">
      <table class="user-table">
          ...
          <tbody>
              <?php
              // Fetch data from the gst table
              $query = "SELECT * FROM hsn_sac";
              $result = mysqli_query($connection, $query);

              if (mysqli_num_rows($result) > 0) {
                  while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr ondblclick=\"window.location.href='edit_hsn_sac.php?id={$row['id']}'\" style='cursor: pointer;'>


                              <td>" . ($row['code'] ?? 'N/A') . "</td>
                              <td>" . ($row['desciption'] ?? 'N/A') . "</td>
                              <td>" . ($row['type'] ?? 'N/A') . "</td>
                              <td>
                                  <button class='btn-warning edit-btn' title='Update HSN/SAC'
                                      onclick=\"window.location.href='edit_hsn_sac.php?id={$row['id']}';\">✏️</button>
                                  <button class='btn-danger' title='Delete this HSN/SAC'
                                      onclick=\"if(confirm('Are you sure you want to delete this record?')) {
                                          window.location.href='delete_hsn_sac.php?id={$row['id']}';
                                      }\">🗑️</button>
                              </td>
                            </tr>";
                  }
