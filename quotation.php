...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $next_quotation_no_result = $conn->query("SELECT CONCAT('QUO', LPAD(COALESCE(MAX(id), 0) + 1, 4, '0')) AS next_quotation_no FROM quotations");
  ...
  echo $quotation_no; // Output example: QUO
    ...
    // Prepare the SQL query to insert data into the quotations table
    ...
    if ($conn->query($insert_quotation) === TRUE) {
$quotation_id = $conn->insert_id;

// Retrieve form data
$products = $_POST['product_name'];
$product_names = $_POST['product_name_actual']; // New: item_name
$quantities = $_POST['quantity'];
$rates = $_POST['rate'];
$gsts = $_POST['product_gst'];
$amounts = $_POST['amount'];
$units = $_POST['unit'];
$unit_values = $_POST['unit_value']; // New: Add this line to retrieve unit values
$igsts = $_POST['igst'];
$cgsts = $_POST['cgst'];
$sgsts = $_POST['sgst'];
...
$product_name = $product_names[$i]; // New: Retrieve item_name
$quantity = $quantities[$i];
$rate = $rates[$i];
$gst = $gsts[$i];
$amount = $amounts[$i];
$unit = $units[$i];
$unit_value = $unit_values[$i]; // New: Retrieve the unit value
$igst = $igsts[$i];
$cgst = $cgsts[$i];
$sgst = $sgsts[$i];
...
$insert_item = "INSERT INTO quotation_items (quotation_id, product_name, product_id, quantity, rate, gst, amount, unit, value, igst, cgst, sgst) VALUES
    ('$quotation_id', '$product_name', '$product_code', '$quantity', '$rate', '$gst', '$amount', '$unit', '$unit_value', '$igst', '$cgst', '$sgst')"; // Added unit_value
    ...
    <form id="invoiceForm" method="POST" action="">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: -25px; width: 100%;">
          <div style="flex-grow: 1; text-align: center;">
              <h1 style="margin-left: 150px;">Sales Quotation</h2>
          </div>
...
function calculateRow(input) {
    ...
    let qty = row.cells[2].querySelector("input").value; // Qty column
    let rate = row.cells[3].querySelector("input").value; // Rate column
