...
<body>
  <div class="leadforhead">
    <h2 class="leadfor">Repeat Follow Ups</h2>
        ...
        if (!empty($fy_codes)) {
            ...
            $query = "SELECT
                      ...
                      ON
                        c.id = f.contact_id
                      WHERE
                        f.lead_followup = 'Repeat' AND f.lead_status != 'Close' AND f.fy_code IN ('$fy_codes_string')";
        } else {
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('downloadExcel').addEventListener('click', function() {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Repeat Followups');
        XLSX.writeFile(wb, 'repeat_followups.xlsx');
      });
      ...
      document.querySelectorAll('.user-table tbody tr').forEach(row => {
        row.addEventListener('dblclick', function(e) {
          ...
          window.location.href = `update_repeat_followup.php?id=${followupId}`;
        });
      });
