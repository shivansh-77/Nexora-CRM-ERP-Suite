...
if (isset($_GET['invoice_id'])) {
    ...
    $check_status_query = "SELECT status, invoice_no FROM invoices WHERE id = ?";
    ...
    try {
        ...
        // 2. Generate invoice number if not already generated
        ...
        // 3. Insert lot items into item_ledger_history (with negative quantities for sales)
        $lot_items_query = "
            SELECT * FROM purchase_order_item_lots
  ...
  while ($item = $lot_items_result->fetch_assoc()) {
      // Make quantity negative for sales
      $negative_quantity = -$item['quantity'];

      $insert_ledger_stmt->bind_param(
          "sssssdssdissdi", // Added 'i' for invoice_id_main (integer)
          $invoice_no,
          $doc_type,
          $entry_type,
          $item['product_id'],
          ...
          $item['rate'],
          $invoice_id // Added invoice_id_main (from $_GET['invoice_id'])
      );
  ...
  $insert_ledger_stmt->close(); // Fixed typo (was $insert_ledger_stmt)

        // 4. Insert into party_ledger (customer instead of vendor)
        $party_ledger_query = "
        ...
        $party_type = 'Customer';
        $doc_type_party = 'Sales Invoice';
        $negative_amount = $invoice['net_amount']; // Negative for sales (credit to customer)
        ...
        $update_invoice_stmt->bind_param("si", $invoice_no, $invoice_id);

        if (!$update_invoice_stmt->execute()) {
            throw new Exception("Error updating invoices: " . $update_invoice_stmt->error);
        }
