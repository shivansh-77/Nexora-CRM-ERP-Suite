...
if (!empty($fy_codes)) {
    ...
    $query = "SELECT id, invoice_no, purchase_order_no, vendor_name, invoice_date, gross_amount, discount, net_amount
...
<body>
    <div class="leadforhead">
        <h2 class="leadfor">Purchase Invoices Lot Details</h2>
...
<button id="downloadExcel" class="btn-primary" title="Export to Excel">
          ...
          <tbody>
              ...
              if (mysqli_num_rows($result) > 0) {
                  while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr ondblclick=\"window.location.href='purchase_lot_tracking_form_display.php?id={$row['id']}'\" style='cursor: pointer;'>
                              ...
                              <td>
                                <button class='btn-secondary' title='Return this Invoice' onclick=\"window.location.href='purchase_invoice_edit.php?id={$row['id']}'\">✏️</button>
                                  <button class='btn-secondary' title='Add Lot Details for this Invoice items' onclick=\"window.location.href='purchase_lot_tracking_form_display.php?id={$row['id']}'\">📋</button>
    ...
    downloadExcelButton.addEventListener('click', function () {
        ...
        XLSX.writeFile(workbook, 'Add_Lot_Purchase_Invoices.xlsx');
    });
});
