...
if (!empty($advance_doc_no)) {
    $stmt = $connection->prepare("SELECT * FROM advance_payments WHERE advance_doc_no = ?");
...
$vendor_details = [];
if (!empty($party_no)) {
    $stmt = $connection->prepare("SELECT contact_person, company_name, mobile_no, whatsapp_no, email_id FROM contact WHERE id = ?");
...
if (!empty($advance_doc_no)) {
    foreach ($advance_details as $index => $row) {
        if ($index === 0) {
            // First entry is the total advance (positive)
            $total_advance = $row['amount'];
        } else {
            // Remaining entries are adjustments (negative)
            $amount_utilized += abs($row['amount']); // make sure it's added as positive value
        }
    }
