...
<link rel="icon" type="image/png" href="favicon.png">
  ...
  <body>
    <div class="leadforhead">
      <h2 class="leadfor">Unit Measurements</h2>
      <div class="lead-actions">
        <div class="search-bar">
          <input type="text" id="searchInput" class="search-input" placeholder="Search...">
          <button class="btn-search" id="searchButton">🔍</button>
        </div>
        <a href="unit_measurement_add.php">
          <button class="btn-primary" id="openModal" title="Add new Unit" data-mode="add">➕</button>
        </a>
      </div>
    </div>
          ...
          <tbody>
              ...
              if (mysqli_num_rows($result) > 0) {
                  while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr ondblclick=\"window.location.href='edit_unit_measurement.php?id={$row['id']}'\" style='cursor: pointer;'>


                              <td>" . ($row['unit'] ?? 'N/A') . "</td>
                              <td>" . ($row['description'] ?? 'N/A') . "</td>
                              <td>
                                  <button class='btn-warning edit-btn' title='Update this Unit'
                                      onclick=\"window.location.href='edit_unit_measurement.php?id={$row['id']}';\">✏️</button>

                                  <button class='btn-danger' title='Delete this Unit'
                                      onclick=\"if(confirm('Are you sure you want to delete this record?')) {
                                          window.location.href='delete_unit_measurement.php?id={$row['id']}';
                                      }\">🗑️</button>
                              </td>
                            </tr>";
                  }
