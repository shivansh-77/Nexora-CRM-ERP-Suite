...
<body>
  ...
  <div class="header" style="position: relative;"> <!-- Added relative positioning -->
      ...
      <h1>Purchase Lot Tracking Form</h1>
      <!-- Added cross button -->
                ...
                <?php if (!empty($items)) { ?>
                    ...
                    ?>
                        <tr class="<?php echo $isEditable ? 'editable' : 'uneditable'; ?>">
                            ...
                            <td>
                                <?php if ($isEditable) { ?>
                                    <a href="purchase_lot_add.php?invoice_id=<?php echo $invoice_id; ?>&invoice_item_id=<?php echo htmlspecialchars($item['id']); ?>&product_id=<?php echo htmlspecialchars($item['product_id']); ?>">
    ...
    function validateAndSave() {
        ...
        fetch('validate_lot_details.php?invoice_id=<?php echo $invoice_id; ?>')
            .then(response => response.json())
            .then(data => {
                if (data.valid) {
                    if (confirm('Are you sure you want to finalize this invoice?')) {
                        // Make a fetch call to finalize_invoice.php
                        fetch('finalize_invoice.php?invoice_id=<?php echo $invoice_id; ?>')
                            .then(response => response.json())
                            .then(result => {
