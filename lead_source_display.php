...
<link rel="icon" type="image/png" href="favicon.png">
  ...
  <body>
    <div class="leadforhead">
      ...
      <button class="btn-primary" id="openModal" data-mode="add" title='Add new Lead Source'>➕</button>
  </a>
          ...
          <tbody>
              ...
              if (mysqli_num_rows($result) > 0) {
                  while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr ondblclick=\"window.location.href='lead_source_edit.php?id={$row['id']}'\" style='cursor: pointer;'>
                              <td>" . ($row['name'] ?? 'N/A') . "</td>
                              <td>" . ($row['status'] ?? 'N/A') . "</td>
                              <td>
                                  <button class='btn-warning edit-btn' title='Update this Lead Source'
                                        onclick=\"window.location.href='lead_source_edit.php?id={$row['id']}';\">✏️</button>
                                  <button class='btn-danger' title='Delete this Lead Source'
