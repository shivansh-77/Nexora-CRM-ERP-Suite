...
<html lang="en" dir="ltr">
  ...
  <body>
    <div class="leadforhead">
      <h2 class="leadfor">Item Sales</h2>
...
$query = "SELECT * FROM item"; // Adjust query as necessary for your database
$result = mysqli_query($connection, $query);

if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        // Assuming 'lot_tracking', 'expiration_tracking', and 'block' are boolean fields
        $lotTracking = $row['lot_tracking'] == 1 ? "Yes" : "No";
        $expirationTracking = $row['expiration_tracking'] == 1 ? "Yes" : "No";
        $block = $row['block'] == 1 ? "Yes" : "No";

        // Fetch the unit from item_add where item_code matches and base_value is 'Active'
        $item_code = $row['item_code'];
        $unitQuery = "SELECT unit_name FROM item_add WHERE item_code = ? AND base_value = 'Active'";
        $stmt = $connection->prepare($unitQuery);
        $stmt->bind_param("s", $item_code);
        $stmt->execute();
        $unitResult = $stmt->get_result();

        $activeUnit = 'N/A'; // Default value in case no active unit is found
        if ($unitResult->num_rows > 0) {
            $unitRow = $unitResult->fetch_assoc();
            $activeUnit = $unitRow['unit_name'];
        }
        ...
        echo "<tr>
                ...
                <td>" . htmlspecialchars($activeUnit) . "</td> <!-- Display active unit -->
                <td>" . ($row['gst_code'] ?? 'N/A') . "</td>
                <td>" . ($row['sales_price'] ?? 'N/A') . "</td>
                <td>" . ($row['location'] ?? 'N/A') . "</td>
                <td>" . ($row['item_type'] ?? 'N/A') . "</td>
                <td>
                <button class='btn-warning edit-btn'
      onclick=\"window.location.href='item_edit.php?id={$row['id']}';\"
      title='Edit this Item'>✏️</button>

  <button class='btn-danger'
      onclick=\"if(confirm('Are you sure you want to delete this record?')) {
          window.location.href='item_delete.php?id={$row['id']}';
      }\"
    ...
    echo "<tr><td colspan='15'>No item records found</td></tr>";
}
?>
