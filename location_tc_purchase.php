...
$tc_query = "SELECT * FROM location_tc WHERE location_id = $location_id AND location_code = '$location_code' AND tc_type = 'Purchase'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($tc_data) {
        ...
        // Insert new record
        $insert_query = "INSERT INTO location_tc (location_id, location_code, tc_type, terms_conditions) VALUES (?, ?, 'Purchase', ?)";
    ...
    header("Location: location_tc_purchase.php?id=$location_id&location_code=$location_code");
    exit();
}
?>
...
<head>
    <meta charset="utf-8">
    <title>Location Purchase Terms and Conditions</title>
...
.back-btn span {
   ...
   /* Ensure the header has a positioning context for the absolute positioned button */
   .header {
       position: relative;
       border-bottom: 2px solid var(--primary-color);
       padding-bottom: 15px;
...
<div class="container">
  <div class="header">
      <h1 class="text-primary">Purchase Terms and Conditions Editor</h1>
      <p class="text-muted">Edit purchase terms and conditions for <?php echo htmlspecialchars($location_data['location'] ?? 'this location'); ?></p>
      <button onclick="window.location.href='locationcard_display.php'" class="back-btn" aria-label="Back">
          <span></span> Back
      </button>
  </div>
    ...
    <div class="editor-container mt-4">
        <h3 class="mb-3"><i class="fas fa-file-contract"></i> Purchase Terms and Conditions</h3>
