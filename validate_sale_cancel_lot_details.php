...
if (isset($_GET['invoice_id'])) {
    ...
    while ($item = $items_result->fetch_assoc()) {
        ...
        while ($lot = $lot_result->fetch_assoc()) {
            ...
            // Check sold quantity in item_ledger_history for this specific lot (only negative quantities)
            $sold_qty_query = "
            ...
            $sold_qty_stmt = $connection->prepare($sold_qty_query);
            $sold_qty_stmt->bind_param(
                "ssss",
                $item['product_id'],
                $lot['location'],
                $lot['lot_trackingid'],
            ...
            $sold_qty_stmt->execute();
            $sold_qty_result = $sold_qty_stmt->get_result();
            $sold_qty_data = $sold_qty_result->fetch_assoc();
            $sold_qty_abs = abs($sold_qty_data['sold_quantity']);

            if ($sold_qty_abs == 0) {
                $lot_validation_errors[] = "{$item['product_name']}: No sales found for Lot {$lot['lot_trackingid']} at {$lot['location']}";
            } elseif ($lot['assigned_quantity'] > $sold_qty_abs) {
                $excess = $lot['assigned_quantity'] - $sold_qty_abs;
                $lot_validation_errors[] = "{$item['product_name']}: Lot {$lot['lot_trackingid']} at {$lot['location']} has {$excess} more than sold quantity";
            }
